...
void ofApp::update() {
	...
	float alpha = 96;
	if (ofGetFrameNum() % 8 == 0 && this->frame_list.size() < 300) {
		...
		case 0:
			...
			mesh.addVertex(glm::vec3(-5, 0, 0));
			mesh.addColor(ofColor(color, alpha));
			mesh.addColor(ofColor(color, alpha));
			frame.addVertex(glm::vec3(5, 0, 0));
			frame.addVertex(glm::vec3(-5, 0, 0));
			frame.addColor(color);
			frame.addColor(color);
			break;
		case 1:
	...
	for (int i = this->mesh_list.size() - 1; i > -1; i--) {
		...
		this->mesh_list[i].addColor(ofColor(this->color_list[i], alpha));
		this->mesh_list[i].addColor(ofColor(this->color_list[i], alpha));

		this->frame_list[i].addVertex(right + rotate_direction);
		this->frame_list[i].addVertex(left + rotate_direction);
		this->frame_list[i].addColor(this->color_list[i]);
		this->frame_list[i].addColor(this->color_list[i]);
